...
${englishText}`;
  ...
  const executeQuizGeneration = async () => {
    ...
    try {
      ...
      for (const item of validItems) {
        ...
        try {
          // work05Service의 generateWork05Quiz 사용 (번역 포함)
      ...
      if (userData?.uid && workTypePoints.length > 0) {
        try {
          ...
          console.log('✅ Work_05 내역 저장 완료 (번역 포함)', generatedQuizzes.length, '개 문제');
        } catch (historyError) {
  ...
  const triggerPrint = (mode: 'no-answer' | 'with-answer') => {
    ...
    printContainer.id = mode === 'with-answer' ? 'print-root-work05-new-answer' : 'print-root-work05-new';
    ...
    root.render(<PrintFormatWork05New quizzes={quizzes} isAnswerMode={mode === 'with-answer'} />);
      ...
      <div>
        <div className="quiz-display no-print">
          <div className="quiz-header">
            <h2 className="no-print">#05. 빈칸(문장) 추론 문제 (총 {quizzes.length}문제)</h2>
  ...
  return (
    <div className="quiz-generator">
      ...
      <button onClick={handleGenerateQuiz} disabled={isLoading} className="generate-button" style={{ marginTop: '20px' }}>
        {items.filter(i => i.text.length >= 10).length > 1 ? `📋 ${items.filter(i => i.text.length >= 10).length}개 문제 일괄 생성` : '📋 빈칸(문장) 문제 생성'}
      </button>
      ...
      <PointDeductionModal
        ...
        workTypeName={`빈칸(문장) 문제 생성 (${items.filter(i => i.text.length >= 10).length}문제)`}
