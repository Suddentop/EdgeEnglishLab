...
${englishText}`;
  ...
  // 본문 → 빈칸 문제/객관식 생성 (AI) - work04Service 사용
  // 이 함수는 호환성을 위해 유지하지만, 실제로는 generateWork04Quiz를 사용합니다.
  async function generateBlankQuizWithAI(passage: string, previouslySelectedPhrases?: string[]): Promise<BlankQuiz> {
    // work04Service의 generateWork04Quiz 함수 사용
    return await generateWork04Quiz(passage, previouslySelectedPhrases);
  }
  ...
  const executeQuizGeneration = async () => {
    ...
    try {
      ...
      const passageGroups = new Map<string, { items: typeof validItems, selectedPhrases: string[] }>();
      
      validItems.forEach(item => {
        const passage = item.text.trim();
        if (!passageGroups.has(passage)) {
          passageGroups.set(passage, { items: [], selectedPhrases: [] });
        }
      ...
      for (const [passage, group] of Array.from(passageGroups.entries())) {
        ...
        for (let i = 0; i < group.items.length; i++) {
          ...
          try {
            ...
            console.log(`  📌 이전 선택 구: ${group.selectedPhrases.length > 0 ? group.selectedPhrases.join(', ') : '없음'}`);
            
            // 이전 선택 구를 포함하여 문제 생성
            const quizData = await generateWork04Quiz(passage, group.selectedPhrases);
            const translation = await translateToKorean(passage);
            
            const quizDataWithTranslation: BlankQuiz = { 
              ...quizData, 
              translation: translation || quizData.translation || '',
              id: item.id
            };
            
            // 생성된 문제의 정답 구를 이전 선택 목록에 추가
            const selectedPhrase = quizData.options[quizData.answerIndex];
            group.selectedPhrases.push(selectedPhrase);
            console.log(`  ✅ 정답 구 "${selectedPhrase}" 선택됨 (이제 제외 목록에 추가됨)`);
            ...
            {quizzes.map((quiz, idx) => {
              ...
              return (
                <div key={quizId} className="quiz-item-card" style={{ 
                  ...
                  <div className="quiz-item-header work04-header" style={{ 
                    marginBottom: '1rem', 
                    display: 'flex', 
                    alignItems: 'center', 
                  ...
                  <div className="problem-instruction work04-instruction" style={{
                    fontWeight: 500, 
                    fontSize: '0.95rem', 
                    background: '#f5f5f5', 
                    color: '#000', 
                  ...
                  <div className="problem-options work04-options" style={{
                    margin: '0 0 1.5rem 0',
                    backgroundColor: '#ffffff',
                    background: '#ffffff',
                    border: '1px solid transparent'
                  ...
                  {quiz.translation && (
                    <div className="translation-section" style={{ marginTop: '2rem' }}>
                      ...
                      <div className="translation-content work04-translation" style={{
                        background: '#F5F5F5',
