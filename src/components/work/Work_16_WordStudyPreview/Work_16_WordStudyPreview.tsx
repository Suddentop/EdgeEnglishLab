...
import './Work_16_WordStudyPreview.css';
...
import { 
  ...
  WordQuizContainerWork16, 
  WordQuestionWork16, 
  WordListTableWork16, 
  PrintFooterWork16, 
  PrintContainerWork16,
  createPaginatedContent,
  getPageTitle,
  PrintModeWork16,
  WordItemWork16,
  WordQuestionWork16Type,
  WordQuizWork16Type
} from './PrintFormat16';
import HistoryPrintWork16 from './HistoryPrintWork16';
import './PrintFormat16.css';

// PrintFormat16의 타입을 사용
type WordItem = WordItemWork16;
type WordQuestion = WordQuestionWork16Type;
type WordQuiz = WordQuizWork16Type;

// 인쇄 모드 타입 (PrintFormat16와 동일하게 유지)
type PrintMode = PrintModeWork16;
...
interface Work_16_WordStudyPreviewProps {
  onClose?: () => void;
}
...
const Work_16_WordStudyPreview: React.FC<Work_16_WordStudyPreviewProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    console.log('🧪 [work16] Work_16_WordStudyPreview 마운트됨', {
  ...
  useEffect(() => {
    if (quiz) {
      console.log('🧪 [work16] quiz 상태 갱신됨', {
  ...
  // 포인트 관련 초기화 제거 (무료 체험용이므로 포인트 불필요)
  ...
  // 단어 수 제한 체크 및 처리 함수
  // 단어 수 제한 체크 및 처리 함수 (무료 체험용: 최대 20개)
  const limitWordsTo20 = (words: WordItem[]): WordItem[] => {
    if (words.length > 20) {
      alert(`무료 체험은 최대 20개까지만 생성 가능합니다.\n입력된 단어: ${words.length}개\n20개까지만 사용됩니다.`);
      return words.slice(0, 20);
    }
    return words;
  };

  // 기존 단어에 새 단어를 추가하고 20개 제한을 적용하는 함수 (무료 체험용)
  ...
  const handleImageChange = async (e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      try {
        ...
        // 20개 초과 시 제한 적용 (무료 체험용)
        const limitedWords = limitWordsTo20(words);
      ...
      if (items[i].type.indexOf('image') !== -1) {
        ...
        if (file) {
          ...
          try {
            ...
            // 기존 단어에 새 단어 추가 (20개 제한 적용, 무료 체험용)
...
7. Extract at least 5-20 words (extract as many as possible, but maximum 20 words for preview)
...
- JSON 형식으로만 응답해주세요`;
  ...
  // 문제 생성 (무료 체험용 - 포인트 차감 없음)
  const handleGenerateQuiz = async () => {
    executeQuizGeneration();
  };

  // 실제 문제 생성 실행 (무료 체험용 - 포인트 차감 없음)
  const executeQuizGeneration = async () => {
    ...
    try {
      ...
      if (inputMode === 'text') {
        ...
        // 20개 초과 시 제한 적용 및 입력창 업데이트 (무료 체험용)
        if (words.length > 20) {
          alert(`무료 체험은 최대 20개까지만 생성 가능합니다.\n입력된 단어: ${words.length}개\n20개까지만 사용됩니다.`);
      ...
      // 문제 생성 내역 저장 제거 (무료 체험용이므로 저장하지 않음)
      
    } catch (err: any) {
      console.error('단어 문제 생성 오류:', err);
      
      // 포인트 환불 로직 제거 (무료 체험용이므로 포인트 없음)
  ...
  const triggerPrint = async (mode: PrintMode) => {
    ...
    console.log('🖨️ [work16] triggerPrint 시작(새 창)', {
      mode,
      wordsCount: quiz.words?.length,
      quizType,
      totalQuestions: quiz.totalQuestions,
    ...
    if (wordsWithoutPos.length > 0) {
      console.log(`🖨️ [work16] 품사 정보가 없는 단어 ${wordsWithoutPos.length}개 발견, 품사 생성은 백그라운드에서 처리...`);
      ...
      addPartOfSpeechToWords(wordsForPrint).then(wordsWithPos => {
        ...
        console.log(`🖨️ [work16] 품사 정보 추가 완료 (백그라운드): ${wordsWithPos.filter(w => w.partOfSpeech).length}개 단어`);
      }).catch(err => {
        console.error('🖨️ [work16] 품사 생성 오류 (백그라운드):', err);
      });
    }

    // HistoryPrintWork16에서 기대하는 데이터 형태로 변환
    ...
    // 디버깅: 품사 정보 확인
    console.log('🖨️ [work16] 인쇄용 데이터 준비 완료 (새 창)', {
    ...
    const adjustFontSizeForPrint = () => {
      const koreanCells = overlay.querySelectorAll('.word-list-table-work16 td:nth-child(3)');
      ...
      <div>
        <div className="quiz-display no-print">
          <div className="quiz-header">
            <h2 className="no-print">단어문제 생성해보기</h2>
          ...
          <div className="quiz-section">
            <div className="problem-instruction" style={{fontWeight:800, fontSize:'1.18rem', background:'#222', color:'#fff', padding:'0.7rem 1.2rem', borderRadius:'8px', marginBottom:'1.2rem', display:'flex', justifyContent:'space-between', alignItems:'center', width:'100%'}}>
              ...
              <span style={{fontSize:'0.9rem', fontWeight:'700', color:'#FFD700'}}>엣지잉글리쉬랩 | https://edgeenglish.net/</span>
            </div>
  ...
  return (
    <div className="quiz-generator" onPaste={handlePaste}>
      ...
      {onClose && (
        <button
          onClick={onClose}
          className="work16-modal-close-btn"
          aria-label="모달 닫기"
        >
          &times;
        </button>
      ...
      <div className="input-section">
        <div className="input-label-row">
          <label htmlFor="word-study-text" className="input-label">
            {inputMode === 'text' ? (
              <>
                영어 단어 텍스트 붙여넣기 : <span style={{color: '#000000', fontWeight: '600', backgroundColor: '#FFF9C4', padding: '0.1rem 0.3rem', borderRadius: '3px'}}>단어문제 생성해보기 (무료체험)</span>에서는 최대 <span style={{color: 'red'}}>20개 단어 이하</span>로 문제생성이 가능합니다.
              </>
            ) : (
              <>
                추출된 단어 목록 : <span style={{color: '#000000', fontWeight: '600', backgroundColor: '#FFF9C4', padding: '0.1rem 0.3rem', borderRadius: '3px'}}>단어문제 생성해보기 (무료체험)</span>에서는 최대 <span style={{color: 'red'}}>20개 단어 이하</span>로 문제생성이 가능합니다.
              </>
            )}
          </label>
...
export default Work_16_WordStudyPreview; 