...
- **🚨 CRITICAL: Personal pronoun + Verb to Personal pronoun + Verb-ing (without be-verb) is ABSOLUTELY FORBIDDEN** — Whenever the structure is "personal pronoun (I/you/we/they/he/she/it) + verb", you MUST NOT simply change the verb to "verb+ing" to make "personal pronoun + verbing". E.g. "They suggest" → "They suggesting", "We work" → "We working", "They rely" → "They relying" are FORBIDDEN (ungrammatical; the verb needs a be-verb). **ALLOWED when transforming the verb in "personal pronoun + verb":** (1) personal pronoun + passive (e.g. They are suggested, We are told), (2) personal pronoun + have/has been + past participle (e.g. They have been suggested), (3) personal pronoun + will + base form (e.g. They will suggest), (4) personal pronoun + would/could/should + base form (e.g. They would suggest). Use these patterns, NOT "pronoun + verb-ing".
...
- **🚨 CRITICAL: 3rd person plural subject (They, We, etc.) + singular verb (-s/-es) is ABSOLUTELY FORBIDDEN** (e.g., "They suggest" → "They suggests", "We believe" → "We believes" — too easy, does not meet exam difficulty).
`;
...
- **Personal pronoun + Verb → Personal pronoun + Verb-ing (without be-verb):** Whenever you have "personal pronoun + verb" (I/you/we/they/he/she/it + verb), you MUST NOT simply change the verb to "verb+ing". E.g. "They suggest" → "They suggesting", "We work" → "We working", "They rely" → "They relying" are FORBIDDEN (ungrammatical). **ALLOWED when transforming the verb:** (1) pronoun + passive (They are suggested), (2) pronoun + have/has been + p.p. (They have been suggested), (3) pronoun + will + base (They will suggest), (4) pronoun + would/could/should + base (They would suggest). Use these, NOT "pronoun + verb-ing".
...
export function validateTransformation(
  ...
  const commonVerbs = ['work', 'go', 'come', 'do', 'make', 'take', 'get', 'give', 'see', 'know', 'think', 'say', 'tell', 'find', 'leave', 'call', 'try', 'ask', 'need', 'want', 'use', 'help', 'play', 'run', 'move', 'like', 'live', 'believe', 'bring', 'happen', 'write', 'sit', 'stand', 'lose', 'pay', 'meet', 'include', 'continue', 'set', 'learn', 'change', 'lead', 'understand', 'watch', 'follow', 'stop', 'create', 'speak', 'read', 'allow', 'add', 'spend', 'grow', 'open', 'walk', 'win', 'offer', 'remember', 'love', 'consider', 'appear', 'buy', 'wait', 'serve', 'die', 'send', 'build', 'stay', 'fall', 'cut', 'reach', 'kill', 'raise', 'pass', 'sell', 'decide', 'return', 'explain', 'develop', 'carry', 'break', 'receive', 'agree', 'support', 'hit', 'produce', 'eat', 'cover', 'catch', 'draw', 'choose', 'rely', 'consist', 'fill', 'prey', 'suggest'];
  const originalIsVerb = commonVerbs.includes(original);
  const transformedIsVerbIng = commonVerbs.some(verb => transformed === verb + 'ing');
  
  // 인칭대명사+동사에서 동사를 단순히 동사+ing로 바꿔 "인칭대명사+동사ing" 만드는 것 금지 (모든 동사 공통)
  if (originalIsVerb && transformedIsVerbIng && !original.startsWith('to ')) {
    return {
      isValid: false,
      errorMessage: `인칭대명사+동사 문장에서 동사를 단순히 "동사+ing"로 바꿔 "인칭대명사+동사ing"로 만드는 것은 절대 금지됩니다. "${originalWord}" → "${transformedWord}"는 비문입니다. 변형 시 인칭대명사+수동태, 인칭대명사+have/has been+과거분사, 인칭대명사+will+동사원형, 인칭대명사+would/could/should+동사원형 등을 사용하세요.`
    };
  }

  // 6a. 3인칭 복수 주어(They, We 등) + 단수동사(-s) 금지 (난이도 너무 낮음)
  const baseToThirdSingular: [string, string][] = [
    ['suggest', 'suggests'], ['believe', 'believes'], ['think', 'thinks'], ['say', 'says'],
    ['tell', 'tells'], ['rely', 'relies'], ['know', 'knows'], ['understand', 'understands']
  ];
  const isForbiddenBaseToS = baseToThirdSingular.some(([b, s]) => original === b && transformed === s);
  if (isForbiddenBaseToS) {
    return {
      isValid: false,
      errorMessage: `3인칭 복수 주어(They, We 등)+동사에서 동사를 단수형(-s/-es)으로 변형("${originalWord}" → "${transformedWord}")하는 것은 절대 금지됩니다. 난이도가 너무 낮습니다. 수동태, have/has been+과거분사, will+동사원형, would/could/should+동사원형 등으로 변형하세요.`
    };
  }
